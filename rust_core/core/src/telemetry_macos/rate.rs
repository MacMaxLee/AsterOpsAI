//! Rate calculation helpers for macOS telemetry.
//!
//! Mirrors `telemetry/rate.rs` — implements gap/reset discipline for
//! counter-based metrics. `elapsed > 2x configured_interval` emits
...
/// Same discipline, specialised for a ticks-idle-over-ticks-total ratio
/// (CPU utilization). Matches `mpstat`'s `%idle` convention: `idle_*` must be
/// the raw idle tick field alone, not idle+iowait.
pub(crate) fn utilization_percent(
    ...
    if total_curr < total_prev || idle_curr < idle_prev {
        return MetricValue::CounterReset {
            reason: "cpu tick counter decreased".to_string(),
        };
    }
