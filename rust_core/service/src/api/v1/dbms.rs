...
/// Unit U72: this handler's own five piggybacked security/config
/// checks (U55-U60) were extracted (U71) and then given a real,
/// independent schedule (`dbms_security_sweep::spawn`) instead of
/// riding along with whatever happens to poll this endpoint — see
/// docs/adr/0078. `gucs` now does exactly what its name says.
pub async fn gucs(
    State(state): State<AppState>,
    Extension(RequestId(request_id)): Extension<RequestId>,
